...
function SidebarNav({
  ...
  return (
    <nav className="flex-1 overflow-y-auto px-3 pt-5 pb-2" aria-label="Main navigation">
      {!collapsed && (
        <p className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[0.12em] text-(--text-quaternary)">
          Navigation
        </p>
      )}
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              className={cn(
                'group relative flex items-center gap-3 rounded-[var(--radius-md)] px-3 py-2.5 text-[13px] font-medium',
            ...
            >
              <span className={cn(
                ...
                active ? 'text-white' : 'text-(--text-tertiary) group-hover:text-(--brand)'
              )}>
                {item.icon}
              </span>
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-(--bg)">
      ...
      <aside
        className={cn(
          'hidden lg:flex lg:flex-col lg:shrink-0 border-r border-(--border) bg-(--surface) transition-[width] duration-250 ease-(--ease)',
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className={cn(
            ...
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[var(--radius-md)] bg-linear-to-br from-(--brand) to-(--brand-hover) text-white shadow-(--shadow-brand)">
            ...
            {!collapsed && (
              <div className="min-w-0">
                ...
                <p className="text-[10px] font-medium text-(--text-quaternary) flex items-center gap-1">
                  <Sparkles size={8} /> {brandLabel}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="border-t border-(--border-light) p-3">
            <button
              ...
              className={cn(
                'flex w-full items-center gap-2.5 rounded-[var(--radius-md)] px-3 py-2 text-[13px] font-medium text-(--text-tertiary)',
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        {/* Topbar */}
        <header className="flex h-(--topbar-height) shrink-0 items-center gap-3 border-b border-(--border) bg-(--surface)/85 px-4 backdrop-blur-md lg:px-6">
          {/* Mobile menu button */}
          <button
            onClick={() => setMobileOpen(true)}
            className="rounded-[var(--radius-md)] p-2 text-(--text-tertiary) transition-colors hover:bg-(--surface-hover) hover:text-(--text-primary) lg:hidden"
          ...
          {/* Breadcrumbs */}
          <nav className="hidden items-center gap-1.5 text-[13px] overflow-hidden md:flex" aria-label="Breadcrumb">
            {breadcrumbs.map((crumb, i) => (
              <span key={i} className="flex items-center gap-1.5 min-w-0">
                {i > 0 && <ChevronRight size={11} className="shrink-0 text-(--text-quaternary)" />}
                {crumb.href ? (
                  <Link
                    href={crumb.href}
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="mx-auto w-full max-w-300 px-4 py-6 md:px-6 lg:px-8 lg:py-8">
            {children}
          </div>
        </main>
      </div>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <button
            ...
            aria-label="Close"
          />
          <aside className="relative h-full w-70 max-w-[85vw] border-r border-(--border) bg-(--surface) shadow-(--shadow-xl) animate-slide-left">
            <div className="flex h-full flex-col">
              <div className="flex items-center justify-between h-(--topbar-height) border-b border-(--border-light) px-5">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 items-center justify-center rounded-[var(--radius-md)] bg-linear-to-br from-(--brand) to-(--brand-hover) text-white shadow-(--shadow-brand)">
                    <GraduationCap size={18} />
                  </div>
                ...
                <button
                  onClick={() => setMobileOpen(false)}
                  className="rounded-[var(--radius-md)] p-1.5 text-(--text-tertiary) hover:bg-(--surface-hover) transition-colors"
                  aria-label="Close"
                >
                  <X size={16} />
                </button>
