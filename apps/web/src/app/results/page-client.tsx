...
const renderResultsSection = (
  title: string,
  description: string,
  results: StudentAssignmentStatus[]
) => {
  if (results.length === 0) {
    return null;
  }
            ...
            <CardContent className="space-y-4">
              <div className="space-y-1 text-sm text-slate-600">
                ...
                {result.visibleStatus === 'PUBLISHED' ? (
                  ...
                  <div>Result will appear here after staff publish it.</div>
                )}
              </div>
              <Link href={`/results/${result.assignmentId}`} className="block">
                <Button className="w-full">
                  {result.visibleStatus === 'PUBLISHED' ? 'Open published result' : 'View waiting status'}
                </Button>
              </Link>
            </CardContent>
          </Card>
...
export default function StudentResultsPageClient() {
  ...
  const awaitingPublication = results.filter((result) => result.visibleStatus === 'SUBMITTED');
