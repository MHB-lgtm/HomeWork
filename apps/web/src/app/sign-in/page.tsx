          ...
          <div className="mt-8 space-y-4">
            {authConfigured ? (
              ...
              <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                Auth is not configured. Set `AUTH_SECRET`, `AUTH_GOOGLE_ID`, and `AUTH_GOOGLE_SECRET`.
              </div>
            )}

            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
              First access is bootstrap-only. Use `AUTH_BOOTSTRAP_SUPER_ADMIN_EMAILS` for the initial
              super-admin email, then add staff users through future membership/admin work.
            </div>
          </div>
        </section>
      </div>
